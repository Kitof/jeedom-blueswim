...
class blueswim extends eqLogic {
   ...
   */
	...
	*/
	public static function cronDaily() {
		self::updateDevices(false, false);
	}
	 	
	private static function tryRequest($request) {
		$client = new Client();
		...
		while($retry > 0) {
			...
			try {
				...
				log::add('blueswim', 'info', "Code erreur: " . $e->getCode() . ". Retry.");
				$force = true;
				$retry--;
				continue;
			}
			if($response->getStatusCode() == 200) break;
			log::add('blueswim', 'info', "Code erreur ".$response->getStatusCode().". Retry.");
			$force = true;
			$retry--;
		}
		if(method_exists($response,'getStatusCode')) return $response;
		else return false;
	} 
	...
	private static function updateDevices($allowCreation=false, $withNotify=false) {
		$blues_list = array();
		...
		$response = self::tryRequest($request);
		if($response === false || $response->getStatusCode() != 200) {
			log::add('blueswim', 'error', "Erreur à la récupération de la liste des piscines.");
			return false;
		}    
		...
		foreach($swimming_pools["data"] as $swimming_pool) {
			...
			$response = self::tryRequest($request);
			if($response === false || $response->getStatusCode() != 200) {
				log::add('blueswim', 'error', "Erreur à la synchronisation des blues.");
				return false;
			}    
			...
			foreach($blues["data"] as $blue) {
				...
				if (!is_object($obj_blue) && $allowCreation) {
					...
					$eqLogic->setConfiguration('lastMeasure',date('Y-m-d H:i:s',strtotime($blue["blue_device"]["last_measure_message"])));
					$eqLogic->setConfiguration('batteryLow',$blue["blue_device"]["battery_low"]);
					$eqLogic->setConfiguration('sleepState',$blue["blue_device"]["sleep_state"]);
					...
					// Update blue
					$obj_blue->setConfiguration('lastMeasure',date('Y-m-d H:i:s',strtotime($blue["blue_device"]["last_measure_message"])));
					$obj_blue->setConfiguration('batteryLow',($blue["blue_device"]["battery_low"])?1:0);
					$obj_blue->setConfiguration('sleepState',$blue["blue_device"]["sleep_state"]);
					$obj_blue->save();
					log::add('blueswim', 'info', "Mise à jour d'un équipement: ".$blueId);
				}
			}
			if($withNotify) {
				event::add('jeedom::alert', array(
	...
	public static function sync() {
		$blues_list = self::updateDevices(true, true);
		return $blues_list;
	}
 ...
 // Fonction exécutée automatiquement après la sauvegarde (création ou mise à jour) de l'équipement 
	public function postSave() {
		$cmd = $this->getCmd(null, 'lastMeasure');
		if (!is_object($cmd)) {
				$cmd = new blueswimCmd();
				$cmd->setLogicalId('lastMeasure');
				$cmd->setIsVisible(1);
				$cmd->setIsHistorized(0);
				$cmd->setName(__('LastMeasure', __FILE__));
		}
		$cmd->setType('info');
		...
		$cmd = $this->getCmd(null, 'batteryLow');
		if (!is_object($cmd)) {
				$cmd = new blueswimCmd();
				$cmd->setLogicalId('batteryLow');
				$cmd->setIsVisible(0);
				$cmd->setIsHistorized(0);
				$cmd->setName(__('BatteryLow', __FILE__));
		}
		$cmd->setType('info');
 ...
 // Fonction exécutée automatiquement après la suppression de l'équipement 
	...
	public static function updateInfo($_eqLogic_id = null, $_cache = null) {
		...
		foreach (self::$_eqLogics as $blueswim) {
			...
			$response = self::tryRequest($request);
			if($response === false || $response->getStatusCode() != 200) {
				log::add('blueswim', 'error', "Erreur à la récupération des données.");
				return false;
			}       
			log::add('blueswim', 'debug', "updateInfo:".$response->getBody());
			$lastMeasures = json_decode($response->getBody(), true);

			$blueswim->checkAndUpdateCmd('state', ($lastMeasures['status']=="BLUE_ASLEEP")?"En hibernation":"Actif", date('Y-m-d H:i:s',strtotime("now")));
			log::add('blueswim', 'info', "state: ".$lastMeasures['status']." en date du ".date('Y-m-d H:i:s',strtotime("now")));
			
			$blueswim->checkAndUpdateCmd('batteryLow', $blueswim->getConfiguration('batteryLow'), date('Y-m-d H:i:s',strtotime("now")));
			$blueswim->batteryStatus(($blueswim->getConfiguration('batteryLow'))?10:100);
			
			log::add('blueswim', 'info', "batteryLow: ".$blueswim->getConfiguration('batteryLow')." en date du ".date('Y-m-d H:i:s',strtotime("now")));
			...
			$blueswim->checkAndUpdateCmd('lastMeasure', date('Y-m-d H:i:s',strtotime($lastMeasures['last_blue_measure_timestamp'])), date('Y-m-d H:i:s',strtotime("now")));
			log::add('blueswim', 'info', "lastMeasure: ".date('Y-m-d H:i:s',strtotime($lastMeasures['last_blue_measure_timestamp']))." en date du ".date('Y-m-d H:i:s',strtotime("now")));
		}
	}
